...
//		}
	...
	public void MoverPeca (Peca peca, Dimension dimension) {
		...
		//Movimento de Rock
		if(peca instanceof Rei && (Math.abs(dimension.width-peca.GetPosicao().GetDimension().width) > 1)) {
			if(dimension.width < peca.GetPosicao().GetDimension().width) {
				MoverPecaSemRestricoes(this.GetPosicaoPorDimension(new Dimension(0, 7*(peca.GetJogador()-1))).GetPeca(), new Dimension(peca.GetPosicao().GetDimension().width-1, peca.GetPosicao().GetDimension().height));
			} else {
				MoverPecaSemRestricoes(this.GetPosicaoPorDimension(new Dimension(7, 7*(peca.GetJogador()-1))).GetPeca(), new Dimension(peca.GetPosicao().GetDimension().width+1, peca.GetPosicao().GetDimension().height));
			}
		}
...
//		this.MostrarTabuleiro();
		System.out.println("00000000000000000000000   " + peca.GetPosicao().GetDimension());
	}
